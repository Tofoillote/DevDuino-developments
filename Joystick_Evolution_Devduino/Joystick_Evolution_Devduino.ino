...
bool DirYd;
bool Bouton;
long OzoneX, OzoneY, Ygauche, Ydroite, ZeroX, ZeroY, CalcY, CalcYgauche, CalcYdroite, CalcXgauche, CalcXdroite, TrimBX, TrimBY, TrimHX, TrimHY;
...
void loop()
{
  ...
  if ( JoyY < TrimBY )  {
      CalcY = 511 * (JoyY - TrimBY ) / TrimBY;
  } else if ( JoyY > TrimHY ) {
      CalcY = 511 * (JoyY - TrimHY ) / ( 1023 - TrimHY )  ;
 }
  else {
    CalcY = 0;  // PWM = 0; Dir = X => Frein moteur.
  }

if ( JoyX > TrimHX ) {
    CalcXgauche = ( 511 * ( JoyX - TrimHX ) / ( 1023 - TrimHX ) );
  }
else if ( JoyX < TrimBX ) {
    CalcXgauche = ( 511 * ( JoyX - TrimBX ) / TrimBX );
  }
else {
    CalcXgauche = 0;  // PWM = 0; Dir = X => Frein moteur.
  }
  
  CalcXdroite = - CalcXgauche;

  
  CalcYgauche = CalcY + CalcXgauche;
  CalcYdroite = CalcY + CalcXdroite;
  ...
  Ygauche = abs(CalcYgauche) / 2;
  Ydroite = abs(CalcYdroite) / 2;
...
display.clear();
  ...
  //display.drawSprite(thanksSprite, 32 + 32*cos(I * M_PI /180 ),32 - 32* sin(I* M_PI /180 ));
    //display.verticalScroll(1, false);
  if (DirYg) { display.fillRectangle(14,30,5, Ygauche/8);} else {display.fillRectangle(14,30 - Ygauche/8 ,5, Ygauche/8);}
  if (DirYd) { display.fillRectangle(110,30,5,Ydroite/8);} else {display.fillRectangle(110,30 - Ydroite/8 ,5,Ydroite/8);}
  display.flush();
